...
class SearchController {
	 ...
	 * @return 検索画面
	 */
	@RequestMapping(value =SEARCH_RETURN, method = RequestMethod.GET)
	public  String search(Model  m){
		...
		int adminbtn = schService.isAdminFlg(loginuser);	//管理者フラグがあれば検温ボタンを押せる
	...
	public String search_info(@ModelAttribute("searchInfo") @Validated SearchInfoForm form, BindingResult result, Model m) {
		...
		boolean display = true;		//テーブルを表示させるか
		boolean errorFlg = false;		//エラーがあるか
		
		int adminbtn = schService.isAdminFlg(loginuser);	//管理者フラグがあれば検温ボタンを押せる
		
		//名前入力チェックでエラーがあったら(未記入はスルー）
		if(result.hasErrors()) {
			errorFlg = true;
		}
	...
	public String logout(@ModelAttribute(THERMO_FORM) ThermoForm FormValue){	
		...
		session.invalidate();	//session一括削除
